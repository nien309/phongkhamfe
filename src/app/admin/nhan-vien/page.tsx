...
export default function EmployeePage() {
  ...
  const [filteredSearch, setFilteredSearch] = useState<NhanVien[] | null>(null);
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      const filtered = employees.filter((emp) =>
        emp.taikhoan.hoten.toLowerCase().includes(keyword) ||
        emp.taikhoan.sdt.includes(keyword)
      );
      ...
      <div className="overflow-x-auto border rounded-lg shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
            ...
            ) : (filteredSearch ?? filteredEmployees).length === 0 ? (
              <tr>
                <td colSpan={5} className="text-center py-6 text-gray-500">
                  Không có nhân viên nào.
                </td>
              ...
              (filteredSearch ?? filteredEmployees).map((emp) => (
                <tr key={emp.id_nhanvien} className="border-t">
                  <td className="px-4 py-3">{emp.id_nhanvien}</td>
                  <td className="px-4 py-3">{emp.taikhoan.hoten}</td>
                  <td className="px-4 py-3">{emp.taikhoan.sdt}</td>
